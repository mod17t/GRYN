...
const ResetPasswordPage = () => {
    ...
    useEffect(() => {
        fetch("http://localhost:8000/api/reset-password/{token}" + searchParams.get('token'), {
                headers: {
       ...
       fetch('http://localhost:8000/api/reset-password', {
        ...
        .then(data => {
            if (data.message || data.error) {
                setError(data.message || data.error || `Une erreur s'est produite.`);
                return;
            }
            setIsSubmitted(true);
        })
            ...
            <section className='min-h-screen mt-10 flex items-center justify-center bg-linear-to-br from-emerald-50 via-green-50 to-teal-50 px-4'>
                <div className='w-full max-w-md'>
                    <div className='flex flex-col items-center mb-8'>
                        ...
                        <p className='text-lg font-normal text-gray-800'>Reinitialiser votre mot de passe</p>
                    </div>
